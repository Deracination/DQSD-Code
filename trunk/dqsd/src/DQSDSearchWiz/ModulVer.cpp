...
BOOL CModuleVersion::GetFileVersionInfo(LPCTSTR modulename)
{
	...
	if (!lib && modulename!=NULL) 
		return FALSE;

	// get module file name
	TCHAR filename[_MAX_PATH];
	if (GetModuleFileName(lib, filename, _MAX_PATH) <= 0)
		return FALSE;
...
//
string CModuleVersion::GetValue(LPCTSTR lpKeyName)
{
	string sVal;
	if (m_pVersionInfo) {
		...
		//
		TCHAR szQuery[ 1024 ];
		_stprintf( szQuery, _T("\\StringFileInfo\\%04x%04x\\%s"),
		...
		LPCTSTR pVal;
		UINT iLenVal;
		if (VerQueryValue(m_pVersionInfo, szQuery,
				(LPVOID*)&pVal, &iLenVal)) {
